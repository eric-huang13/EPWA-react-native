...
const CONNECTOR_SIZE_DEFAULT = 14;
...
export class DragResizeBlock extends Component {
  ...
  constructor(props) {
    ...
    this.size = size || CONNECTOR_SIZE_DEFAULT;
    this.state = {
      isSelected: false,
      x,
      y,
    ...
    this.connectorsMap[CONNECTOR_CENTER] = {
      ...
      border: {color: 'transparent', width: 0, radius: 0},
      calculateWidth: (width) => {return width - this.size * 2},
      calculateHeight: (height) => {return height - this.size * 2},
    ...
    return connectors.map((connectorType) => {
      return (
        <Connector
          ...
          size={this.size}
          border={
            this.connectorsMap[connectorType].border
            || {color: borderColor, width: 1, radius: this.size / 2}
          }
          background={this.connectorsMap[connectorType].background || borderBackground}
          width={
            this.connectorsMap[connectorType].calculateWidth
            && this.connectorsMap[connectorType].calculateWidth(w)
          }
          height={
            this.connectorsMap[connectorType].calculateHeight
            && this.connectorsMap[connectorType].calculateHeight(h)
          }
