    ...
    if (prevProps.caregiver.loading && !caregiver.loading) {
      if (!caregiver.message.success) {
        ...
        if (errorMessage.error == "user_unknown") {
          Alert.alert(
            t("caregiverAlertTitle"),
            t("caregiverAlertUserUnknown"),
            [
              {
                text: t("OK"),
                onPress: () => {
                  this.setFocusTextInput();
                }
              }
            ],
            { cancelable: false }
